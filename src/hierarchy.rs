...
pub fn target_from_url(config: &Config, input: &str) -> Result<Target> {
    if let (Ok(url), Some(base)) = (url::Url::parse(input), &config.gitlab_url)
        && url.origin() == base.origin()
    {
        return Err(Error::new(
            "input",
            "GitLab does not use native hierarchy; create ordinary issues and record explicit parent/child delivery contracts",
        ));
    }
    let target = Target::from_url(config, input)
        .map_err(|_| Error::new("input", "Expected a GitHub issue URL"))?;
    if target.platform != Platform::Github {
        return Err(Error::new(
            "input",
            "GitLab does not use native hierarchy; create ordinary issues and record explicit parent/child delivery contracts",
        ));
...
impl Hierarchy<'_> {
    fn github(&self) -> Result<()> {
        if self.parent.platform != Platform::Github {
            return Err(Error::new(
                "input",
                "Native hierarchy commands support GitHub issues only",
            ));
        }
        Ok(())
    }
